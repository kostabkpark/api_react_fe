...
import CommentTr from "./CommentTr.jsx";
...
function View(props) {
  ...
  const getCommentList = async () => {
    ...
    const res = await axiosInstance.get("/api/comments", { params: param });
  ...
  const saveComment = async () => {
    ...
    const res = await axiosInstance.post("/api/comments", param);
  ...
  const delComment = async (no) => {
    if (window.confirm("댓글을 삭제하시겠습니까?")) {
      ...
      const res = await axiosInstance.delete(`/api/comment/${no}`);
      if (res.data.result === "success") {
        alert("정상적으로 삭제되었습니다.");
        getCommentList();
      }
  ...
  const goDelete = async (e) => {
    if (window.confirm("삭제하시겠습니까?")) {
      ...
      // no: Number(no),
      // });
      const res = await axiosInstance.delete(`/api/replies/${no}`);  
      if (res.data.result === "success") {
        alert("정상적으로 삭제되었습니다.");
        navigate("/board/list");
      }
